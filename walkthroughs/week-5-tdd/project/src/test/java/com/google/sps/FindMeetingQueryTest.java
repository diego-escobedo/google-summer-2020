...
public final class FindMeetingQueryTest {
  ...
  private static final String PERSON_B = "Person C";
  ...
  @Test
  public void everyAttendeeIsConsideredOptionalOccupiedAllDay() {
    // MAndatory have an available timeslot, but optional is busy all day
    ...
    Collection<Event> events = Arrays.asList(
        ...
        new Event("Event 2", TimeRange.fromStartDuration(TIME_0900AM, DURATION_30_MINUTES),
            Arrays.asList(PERSON_B)),
        new Event("Event 3", TimeRange.fromStartEnd(TIME_0000AM, TIME_1159PM, true),
  ...
  @Test
  public void everyAttendeeIsConsideredOptionalOccupiedMiddleOfDay() {
    // Basedon everyAttendee is considered, but now the optional has a meeting during one of the returned slots
    Collection<Event> events = Arrays.asList(
        new Event("Event 1", TimeRange.fromStartDuration(TIME_0800AM, DURATION_30_MINUTES),
            Arrays.asList(PERSON_A)),
        new Event("Event 2", TimeRange.fromStartDuration(TIME_0900AM, DURATION_30_MINUTES),
  ...
  @Test
  public void justEnoughRoomOptionalNotFreeAtRightTime() {
    // Optional has a meeting at exactly the time the mandatory is available
  ...
  public void noMandatoryWithGaps() {
    // No mandatory people, and the optional people have space for meetings

    Collection<Event> events = Arrays.asList(
        new Event("Event 1", TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_0830AM, false),
            Arrays.asList(PERSON_A)),
        new Event("Event 2", TimeRange.fromStartEnd(TIME_0900AM, TIME_1100AM, true),
            Arrays.asList(PERSON_A)),
        new Event("Event 3", TimeRange.fromStartEnd(TIME_1200AM, TimeRange.END_OF_DAY, true),
            Arrays.asList(PERSON_A)),
        new Event("Event 4", TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_0830AM, false),
            Arrays.asList(PERSON_B)),
        new Event("Event 5", TimeRange.fromStartEnd(TIME_0900AM, TIME_1100AM, true),
            Arrays.asList(PERSON_B)),
        new Event("Event 6", TimeRange.fromStartEnd(TIME_1200AM, TimeRange.END_OF_DAY, true),
            Arrays.asList(PERSON_B)));

    MeetingRequest request = new MeetingRequest(new Arraylist<String>(), DURATION_30_MINUTES);
    request.addOptionalAttendee(PERSON_A)
    request.addOptionalAttendee(PERSON_B)

    Collection<TimeRange> actual = query.query(events, request);
    Collection<TimeRange> expected = Arrays.asList(TimeRange.fromStartDuration(TIME_0830AM, DURATION_30_MINUTES),
            TimeRange.fromStartDuration(TIME_1100AM, DURATION_60_MINUTES));
