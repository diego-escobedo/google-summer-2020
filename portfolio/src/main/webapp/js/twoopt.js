  ...
  while (iter < iterlimit) {
    ...
    for (i = 1; i < existing_route.length - 2; i++) {
      for (k = i + 1; k < existing_route.length - 1; k++) {
        ...
        iterlimit += 1; // keep track of all the loops
        var new_distance = calculateTotalDistance(new_route, distmat);
        if (new_distance < best_distance) {
          existing_route = new_route;
          best_distance = new_distance;
          better_found = true;
          break; //step 1 to get to goto
        }
      }
      if (better_found === true) {
        break; //step 2 to get to goto
      }
    }
    if (better_found === false) {
      break; //early stopping.... if no one better is found, no sense to keep going
    }
  }
